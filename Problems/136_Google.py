...
"""
This would take O(M N^3) time and O(N) space, since we keep one row in memory (as the cache), and we run a double for loop over each row to calculate the area.
"""
