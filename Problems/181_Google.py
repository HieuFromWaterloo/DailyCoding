...
"""
This takes O(n^3) time, since we have to build up the matrix (which is O(n2) time), and also create a new list with size up to O(n) for each inner loop of the cache. This also takes O(n^2) space.
"""
