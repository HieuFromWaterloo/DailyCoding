...
Recall that because we use the decimal system, any n-digit integer x1x2...xn can be represented as

[x1* 10^(n-1)]+ [x2 * 10^(n-2)] + ... + [xn * 10^0].
...
Get the first digit: int // 10^n
Get the last digit: n % 10
Remove the first digit: int % 10^n
