...
"""
This is a map with a list of sorted keys. To find out the nearest previous time we use the binary search algorithm provided by the *bisect*.[https://docs.python.org/3/library/bisect.html]

Any write operation on this map wipes the key's cache, causing a full sort of the keys on the next get call, which in python's *TimSort* averages as O(n log n) complexity. [https://en.wikipedia.org/wiki/Timsort]
